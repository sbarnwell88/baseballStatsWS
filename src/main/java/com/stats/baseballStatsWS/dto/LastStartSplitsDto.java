package com.stats.baseballStatsWS.dto;

public class LastStartSplitsDto {

    private int value;
    private String opponent_id;
    private String game_id;
    private String game_scheduled;
    private int win;
    private int loss;
    private int save;
    private int svo;
    private int start;
    private int play;
    private int complete;
    private int ip_1;
    private float ip_2;
    private int h;
    private int runs;
    private int er;
    private int hr;
    private int bb;
    private int ibb;
    private float oba;
    private float era;
    private int ktotal;
    private int team_win;
    private int team_loss;

    public int getValue() {
        return value;
    }

    public void setValue(int value) {
        this.value = value;
    }

    public String getOpponent_id() {
        return opponent_id;
    }

    public void setOpponent_id(String opponent_id) {
        this.opponent_id = opponent_id;
    }

    public String getGame_id() {
        return game_id;
    }

    public void setGame_id(String game_id) {
        this.game_id = game_id;
    }

    public String getGame_scheduled() {
        return game_scheduled;
    }

    public void setGame_scheduled(String game_scheduled) {
        this.game_scheduled = game_scheduled;
    }

    public int getWin() {
        return win;
    }

    public void setWin(int win) {
        this.win = win;
    }

    public int getLoss() {
        return loss;
    }

    public void setLoss(int loss) {
        this.loss = loss;
    }

    public int getSave() {
        return save;
    }

    public void setSave(int save) {
        this.save = save;
    }

    public int getSvo() {
        return svo;
    }

    public void setSvo(int svo) {
        this.svo = svo;
    }

    public int getStart() {
        return start;
    }

    public void setStart(int start) {
        this.start = start;
    }

    public int getPlay() {
        return play;
    }

    public void setPlay(int play) {
        this.play = play;
    }

    public int getComplete() {
        return complete;
    }

    public void setComplete(int complete) {
        this.complete = complete;
    }

    public int getIp_1() {
        return ip_1;
    }

    public void setIp_1(int ip_1) {
        this.ip_1 = ip_1;
    }

    public float getIp_2() {
        return ip_2;
    }

    public void setIp_2(float ip_2) {
        this.ip_2 = ip_2;
    }

    public int getH() {
        return h;
    }

    public void setH(int h) {
        this.h = h;
    }

    public int getRuns() {
        return runs;
    }

    public void setRuns(int runs) {
        this.runs = runs;
    }

    public int getEr() {
        return er;
    }

    public void setEr(int er) {
        this.er = er;
    }

    public int getHr() {
        return hr;
    }

    public void setHr(int hr) {
        this.hr = hr;
    }

    public int getBb() {
        return bb;
    }

    public void setBb(int bb) {
        this.bb = bb;
    }

    public int getIbb() {
        return ibb;
    }

    public void setIbb(int ibb) {
        this.ibb = ibb;
    }

    public float getOba() {
        return oba;
    }

    public void setOba(float oba) {
        this.oba = oba;
    }

    public float getEra() {
        return era;
    }

    public void setEra(float era) {
        this.era = era;
    }

    public int getKtotal() {
        return ktotal;
    }

    public void setKtotal(int ktotal) {
        this.ktotal = ktotal;
    }

    public int getTeam_win() {
        return team_win;
    }

    public void setTeam_win(int team_win) {
        this.team_win = team_win;
    }

    public int getTeam_loss() {
        return team_loss;
    }

    public void setTeam_loss(int team_loss) {
        this.team_loss = team_loss;
    }
}
